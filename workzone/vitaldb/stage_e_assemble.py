#!/usr/bin/env python3
"""
Stage E - VitalDB 4-partition EHR assembler.

Episode bounds = [anestart_ms, aneend_ms] from meta.json (set in Stage B).
"""
...
SUMMARY_JSON = "/labs/hulab/mxwang/Physio_Data/workzone/outputs/vitaldb/stage_e_summary.json"
...
def process_entity(row, out_root, resume):
    ...
    try:
        ...
        adm_start = meta.get("anestart_ms")
        adm_end   = meta.get("aneend_ms")
        partitions = split_events(combined, time_ms,
                                  episode_start_ms=int(adm_start) if adm_start is not None else None,
                                  episode_end_ms=int(adm_end)     if adm_end   is not None else None,
        ...
        meta.update({
            ...
            "wave_end_pad_ms":   WAVE_END_PAD_MS,
            "admission_start_ms": int(adm_start) if adm_start is not None else None,
            "admission_end_ms":   int(adm_end)   if adm_end   is not None else None,
...
def main():
    ...
    cohort = pl.read_parquet(COHORT_PARQUET).unique("entity_id", keep="first").select(["entity_id"])
