#!/usr/bin/env python3
"""
Stage F-1 - VitalDB manifest + splits (70/15/15, seed=42, grouped by subjectid).
"""
...
def validate_entity(entity_dir: Path):
    ...
    entry.update({
        ...
        "n_seg":     n_seg,
        "source_dataset": meta.get("source_dataset", "vitaldb"),
    })
    ...
    if not time_path.exists():
        ...
        if len(t) > 1 and not np.all(np.diff(t) > 0):
            errors.append("time_ms not monotonic")
        if len(t):
...
def group_split(groups, ratios, seed):
    ...
    buckets = {"train": keys[:n_train],
               "val":   keys[n_train:n_train + n_val],
               "test":  keys[n_train + n_val:]}
    return {k: sorted(sum((groups[g] for g in v), [])) for k, v in buckets.items()}
...
def main():
    ...
    for e in valid:
        s = str(e.get("subjectid") or "unknown")
        groups.setdefault(s, []).append(e["entity_id"])
    splits = group_split(groups, ratios, args.seed)

    pretrain_json = {"seed": args.seed, "ratios": list(ratios),
                     "group_by": "subjectid", "stratify_by": None,
                     "n_unique_subjects": len(groups),
    ...
    def build_list(eids):
        return [[str(out_root / e), str(entry_by_eid[e].get("subjectid", "")),
