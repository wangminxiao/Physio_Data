#!/usr/bin/env python3
"""
Stage D — MC_MED labs extraction from labs.csv (752 MB).

Phase 1: Polars streaming scan of labs.csv filtered to cohort CSNs.
  Map Component_name -> var_id (see COMPONENT_TO_VAR_ID). Parse
  Component_value; drop non-numeric. Apply physio range filter.
  Write:
    workzone/outputs/mcmed/stage_d_labs_combined.parquet

Phase 2: partition by CSN. For each entity with Stage B meta.json:
  Write {entity}/labs_events.npy as structured (time_ms, var_id, value).
  Update meta with `labs` section + stage_d_version=1.

Use Result_time as the lab event timestamp.
"""
import argparse
import json
import logging
...
def phase1(cohort_csns: list[int]) -> dict:
    ...
    lf = pl.scan_csv(LABS_CSV, low_memory=True, infer_schema_length=10000,
                     ignore_errors=True)
    names = list(COMPONENT_TO_VAR_ID.keys())
    df = (
        lf.filter(pl.col("CSN").cast(pl.Int64, strict=False).is_in(cohort_csns))
          .filter(pl.col("Component_name").is_in(names))
          .select([
              pl.col("CSN").cast(pl.Int64, strict=False).alias("csn"),
              iso_z_to_ms("Result_time").alias("time_ms"),
              pl.col("Component_name"),
              pl.col("Component_value").cast(pl.Utf8).str.strip_chars()
                .str.strip_chars("<>=~%+ ")
                .cast(pl.Float64, strict=False).alias("value_f"),
          ])
          .with_columns([
              pl.col("Component_name").replace_strict(COMPONENT_TO_VAR_ID,
...
def phase2(cohort_df: pl.DataFrame, out_root: str, resume: bool) -> list[dict]:
    print(f"[D2] reading combined parquet")
    combined = pl.read_parquet(COMBINED_PARQUET)
    print(f"[D2] rows={combined.height:,}  unique_csns={combined['csn'].n_unique()}")
...
def main():
    ...
    if args.phase in ("1", "all"):
        if args.redo_phase1 or not os.path.exists(COMBINED_PARQUET):
            ...
            print(f"[D1] skipping (combined exists: {COMBINED_PARQUET})")

    if args.phase in ("2", "all"):
        if not os.path.exists(COMBINED_PARQUET):
            print("[D2] needs phase 1 first"); sys.exit(1)
        df = cohort
        if args.entity_id:
        ...
        print(f"[D2] {df.height} entities  resume={not args.no_resume}")
        t0 = time.time()
