#!/usr/bin/env python3
"""
Stage F-1 - MOVER/SIS manifest + patient-level splits (70/15/15, seed=42).

No MRN grouping needed: each PID is one surgery = one entity = one "patient"
(patient-level id). We still shuffle by PID to be explicit.
"""
...
def validate_entity(entity_dir: Path):
    ...
    entry.update({
        "pid": meta.get("pid"),
        "n_seg": n_seg,
        "source_dataset": meta.get("source_dataset", "mover_sis"),
    })
...
def simple_split(ids: list[str], ratios, seed: int):
...
def main():
    ...
    splits = simple_split([e["entity_id"] for e in valid], ratios, args.seed)

    pretrain_json = {"seed": args.seed, "ratios": list(ratios),
                     "group_by": "pid", "stratify_by": None,
    ...
    def build_list(eids):
        return [[str(out_root / e), str(entry_by_eid[e].get("pid", "")),
