...
MIN_SECONDS_PRESENT = 24             # 24/30 seconds covered (80%)
MAX_NAN_RATIO = 0.20
DEFAULT_WORKERS = 16
...
def process_entity(row: dict, out_root: str = OUT_ROOT) -> dict:
    ...
    first_ms = all_secs[0]
    last_ms = all_secs[-1]
    # Window starts every 30 s from first_ms (rounded to second)
    ...
    for t_start in win_starts:
        ...
        if i_sec < MIN_SECONDS_PRESENT:
            # Too sparse II: emit all-NaN for this window
    ...
    meta = {
        ...
        "channels": {
            "PLETH40": {"sample_rate_hz": PLETH_FS, "shape": list(pleth40.shape),
                        "dtype": "float16",
                        "source": f"SIS XML PLETH @ {PLETH_SRC_FS} Hz, resample_poly(2,5)"},
            "II120":   {"sample_rate_hz": II_FS, "shape": list(ii120.shape),
                        "dtype": "float16",
                        "source": f"SIS XML ECG1 @ {II_SRC_FS} Hz, resample_poly(2,5); NaN-filled when coverage<80%"},
        },
