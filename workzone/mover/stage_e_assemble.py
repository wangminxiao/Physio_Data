#!/usr/bin/env python3
"""
Stage E - MOVER/SIS 4-partition EHR assembler.

Per entity with Stage B (+C +D) on disk:
  - Load time_ms.npy + vitals_events.npy + labs_events.npy.
  - Concat + sort.
  - split_events(episode_start=or_start_ms, episode_end=or_end_ms).
  - Write ehr_{baseline,recent,events,future}.npy + meta update.
"""
...
SUMMARY_JSON = "/labs/hulab/mxwang/Physio_Data/workzone/outputs/mover/stage_e_summary.json"
...
def process_entity(row, out_root, resume):
    ...
    st = {"entity_id": pid, "status": "pending",
    ...
    try:
        ...
        adm_start = int(row["or_start_ms"]) if row.get("or_start_ms") is not None else None
        adm_end   = int(row["or_end_ms"])   if row.get("or_end_ms")   is not None else None
...
def main():
    ...
    cohort = (pl.read_parquet(COHORT_PARQUET)
              ...
              .select(["entity_id", "or_start_ms", "or_end_ms"]))
